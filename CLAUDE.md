...
1. **Google sign-in** is coded but disabled — Supabase reports `google: false`
   until OAuth credentials are configured. Email + password for day-to-day
   sign-in; a 6-digit email code confirms new signups and password resets.
   Google is the fast path.

 ...
 * Served at /preview/practice and framed by the landing hero iPad.
 */
function pickQuestion() {
  // Prefer a short stem so it stays readable when scaled into the hero.
  const preferred = QUESTIONS.find(
    (q) =>
      q.opts?.length === 5 &&
      q.q.length < 70 &&
      (q.deck?.includes("Physiology") ||
        q.subject === "Physiology" ||
        /pH|cardiac|receptor/i.test(q.q)),
  );
...
export default function PracticePreview() {
  ...
  const topic = q.deck || q.subject || "Practice";
